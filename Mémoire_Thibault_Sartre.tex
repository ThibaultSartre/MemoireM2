...
Je vais maintenant vous présenter comment on pourrait utiliser les graphes de flot de contrôle dans le refactoring et plus précisément pour détecter du code mort.\\
...
C'est à dire de trouver au moins un ensemble de combinaisons de valeurs qui permet de passer toutes les conditions du programme. Cette technique pourrait donc être utilisé dans le refactoring pour détecter du code mort. Puisque dans le cas où il n'existe aucun ensemble de combinaisons de valeur permettant de satisfaire toutes les conditions, alors on peut détecter que dans le programme une partie du code est inatteignable et qu'il est donc mort.\\
...
Imaginons que dans l'exemple une variable y soit générer aléatoirement entre -100 et 100.
L'exemple ci-dessus est très simple mais à pour but de montrer qu'il est possible de détecter du code mort.\\
On a donc une variable x à laquelle on affecte le carré d'une variable y, or le carré de n'importe quel entier est positif ou nulle.\\ Si l'on regarde le mini programme, on peut facilement comprendre que jamais on ne pourra passer par le else car x ne peut pas être négatif. Mais imaginons que ce soit une programme beaucoup plus compliqué.\\ On a alors le graphe de flots de contrôle, qui à l'aide d'un algorithme permettant de vérifier toutes les décision possible, va nous dire qu'il est impossible de passer la condition x est négatif. Le graphe de flots de contrôle nous permettrait donc de détecter du code mort en étudiant les décisions possibles d'un programme.\\
En revanche la complexité de l'algorithme qui détecte toute les décisions possibles est exponentiel en fonction du nombre de condition présent dans le programme.\\
Pour contourner ça, il serait judicieux de l'appliquer sur des morceaux de programme pour diminuer le nombre de conditions traitées à la fois pour pouvoir obtenir un résultat dans des délais acceptables.\\
