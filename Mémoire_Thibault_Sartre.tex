...
Dans un autre article on nous dit qu'en utilisant les graphes de flots de contrôle couplé à l'exécution symbolique, il est possible de détecté des chemins infaisable\cite{ref20}.\\
...
Cela pourrait être un point bloquant si on veut utiliser cette solution couplé aux graphes de flots de contrôle pour faire du refactoring. Il faudrait se limiter à des programmes avec lesquels on peut générer un arbre d'exécution symbolique fini.\\
...
Il permet donc de trouver des classes, méthodes ou bien des champs qui ne sont utilisés nul part.\\
UCDetector est capable réduire la visibilité du code en proposant de changer la visibilité des méthodes ou autre à "protected", "default" ou bien "private" selon les différents cas.\\
Il est aussi possible de détecter si un champs ou une méthode peut être déclarer en "final".\\
En revanche il existe des cas ou UCDetector peut se tromper(il ne s'agit que de suggestion)\cite{ref21} dans l'indication de code mort si le code est utilisé par :\\
...
On retrouve énormément d'outils permettant soit de générer des graphes de flots de contrôle afin de pouvoir mieux visualiser le fonctionnement de son programme.\\
On trouve aussi des plugins ou bien des projets qui permette de faire du refactoring notamment AutoRefactor qui est le seul logiciel de refactoring existant avec l'ambition d'utiliser les graphes de flots de contrôle dans le refactoring.\\ 
