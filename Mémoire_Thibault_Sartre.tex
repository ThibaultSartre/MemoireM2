...
Dans ce mémoire, nous allons analyser différentes techniques de refactoring. Puis nous allons étudier rapidement le principe des flots de contrôle.\\ Enfin on étudiera s'il est possible de faire du refactoring à l'aide des flots de contrôle pour le langage Java.\\
...
Lorsque l'on code beaucoup de fonctionnalités dans un même programme, il arrive qu'au bout d'un moment, on se rendent compte que , par exemple, on a placé une fonctionnalités dans la mauvaise classe et qu'on ne sache pas comment faire pour la déplacer sans risquer de casser tout le code.\\
...
\subsection{Déplacement de variable}
s
\subsection{Extraction de classe}
s
\subsection{Cacher la délégation}
s
\subsection{Ajout de méthode étrangère}
s
...
s
\subsection{Remplacer les données par des objets}
s
\subsection{Passer du stockage par valeur en référence}
s
\subsection{Remplacer les tableaux par des objets}
s
\subsection{Changer une association unidirectionnelle en bidirectionnelle}
s
\subsection{Remplacer les numéro magiques en constantes}
s
...
Dans les programmes, il n'est pas rare de trouver des expressions conditionnels qui possèdent une logique compliquées avec beaucoup de condition. Ce type de refactoring va permettre de simplifier toutes ces expressions.
...
Ces techniques de refactoring on pour but de simplifier l'appel de méthode ainsi que de le rendre plus facile à comprendre.
